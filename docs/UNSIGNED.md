# Running unsigned Atem Overseer builds
...
Atem Overseer listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of Atem Overseer**
...
domain. Atem Overseer needs this to serve the dashboard and poll ATEM switchers on the LAN.

If you deny it (or the prompt appears behind another window and times out), switchers stay greyed out and the dashboard won't load from another machine
...
**Private**. If Atem Overseer isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "Atem Overseer" -Direction Inbound -Program "C:\Path\To\Atem Overseer.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*Atem Overseer*" | Remove-NetFirewallRule
```
...
```sh
codesign --force --deep --sign - "/Applications/Atem Overseer.app"
```

Verify it took:

```sh
codesign -dv --verbose=4 "/Applications/Atem Overseer.app"
spctl -a -vv "/Applications/Atem Overseer.app"   # still reports "rejected" — ad-hoc is not notarization
```
